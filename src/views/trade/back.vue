...
import { reqAddressInfo, reqTradeNo, postOrder } from '/@/api/sys/payment'
import { AddressInfo, Order, OrderDetail } from '/@/api/sys/model/payModel'
import { useCarStoreWithOut } from '@/store/modules/shopcart';
import { useRouter } from 'vue-router';

const currentAddressInfoIndex = ref(0)
const addressInfos = reactive([] as Array<AddressInfo>)
const orderComment = ref('')
const cartStore = useCarStoreWithOut()
...
onMounted( () => {
  cartStore.loadList();
  addressInfos.push(...( reqAddressInfo()).data as Array<AddressInfo>)
  tradeNo.value = ( reqTradeNo()).data.tradeNo
})
